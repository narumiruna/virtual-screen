...
enum DialogPresenter {
    static func confirm8K() -> Bool {
        let alert = makeAlert(
            ...
            message: String(
                ...
                defaultValue: "8K virtual displays use substantial memory and GPU resources. Multiple high-resolution displays may reduce system stability."
            ),
            style: .warning
        )
    ...
    static func requestName(currentName: String) -> String? {
        let alert = makeAlert(
            ...
            message: String(localized: "dialog.rename.message", defaultValue: "Enter a name for this virtual display."),
            style: .informational
        )
        let field = NSTextField(string: currentName)
        field.frame = NSRect(x: 0, y: 0, width: 280, height: 24)
        field.placeholderString = String(localized: "dialog.rename.placeholder", defaultValue: "Display name")
    ...
    static func showLaunchAtLoginApproval(openSettings: () -> Void) {
        let alert = makeAlert(
            title: String(localized: "dialog.loginApproval.title", defaultValue: "Approve Launch at Login"),
            message: String(
                localized: "dialog.loginApproval.message",
                defaultValue: "macOS requires approval before Virtual Screen can start at login. Approve it in System Settings > General > Login Items."
            ),
            style: .informational
        )
        alert.addButton(withTitle: String(localized: "dialog.openSettings", defaultValue: "Open Login Items"))
