...
private final class CoreGraphicsVirtualDisplayConnection: VirtualDisplayConnection, @unchecked Sendable {
...
final class CoreGraphicsVirtualDisplayBackend: VirtualDisplayBackend {
    private let workQueue = DispatchQueue(label: "com.narumi.VirtualScreen.backend", qos: .userInitiated)
        ...
        return try await withCheckedThrowingContinuation { continuation in
            workQueue.async { [logger] in
                do {
                    ...
                    logger.info("Connected virtual display \(profile.id.uuidString, privacy: .public) as \(handle.displayID)")
                    ...
                    logger.error("Could not connect virtual display: \(mappedError.localizedDescription, privacy: .public)")
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
            workQueue.async { [logger] in
                do {
                    ...
                    let mappedError = VirtualDisplayBackendError.from(error)
                    logger.error("Could not switch display mode: \(mappedError.localizedDescription, privacy: .public)")
                    continuation.resume(throwing: mappedError)
                }
            }
        }
